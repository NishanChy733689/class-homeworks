...
auth.onAuthStateChanged(function (user) {
  ...
  userRef.on('value', function (snapshot) {
    ...
    document.getElementById('profile-pic').src =
      userInfo.profilePic || 'https://randomuser.me/api/portraits/men/32.jpg';
    ...
    document.getElementById('nav-name').innerText = userInfo.name
    ...
    document.getElementById('crd').textContent = userInfo.credits || 0;
    console.log('Set credits:', userInfo.credits);

    document.getElementById('profile-progress').textContent = userInfo.progress
      ? `${userInfo.progress}%`
      : '0%';
    console.log('Set profile-progress:', userInfo.progress ? `${userInfo.progress}%` : '0%');
      ...
      leaderboardList.innerHTML += `
        <li class="list-group-item d-flex align-items-center justify-content-between">
          <span>
            <img src="${
              entry.profilePic ||
              'https://ui-avatars.com/api/?name=' + encodeURIComponent(entry.name || '')
            }" class="rounded-circle me-2" width="36" height="36" />
            <span class="fw-bold">#${idx + 1} ${entry.name || ''}</span>
            <span class="text-muted ms-2">${entry.grade ? 'Grade ' + entry.grade : ''}</span>
          </span>
          <span class="fw-bold">${entry.points || 0}</span>
        </li>
      `;
